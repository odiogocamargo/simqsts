...
// entity_type recebido  →  tabela deste banco
...
Deno.serve(async (req) => {
  ...
  // 1) Valida assinatura HMAC-SHA256 do corpo bruto
  ...
  if (evt.operation === "delete") {
    ...
    const { error } = await supa.from(table).upsert(evt.payload);
    if (error) {
      return new Response(JSON.stringify({ error: error.message, table, payload_keys: Object.keys(evt.payload ?? {}) }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
