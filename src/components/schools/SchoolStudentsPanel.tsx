...
export function SchoolStudentsPanel({ school, onBack }: SchoolStudentsPanelProps) {
  ...
  const { data: students, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: profiles } = await supabase.from("profiles").select("id, full_name, cpf, whatsapp").in("id", userIds);
      const profileMap = new Map(profiles?.map((p: any) => [p.id, p]) || []);
      return data.map((d: any) => ({ ...d, profile: profileMap.get(d.user_id) || null })) as StudentRow[];
    },
  });

  const { data: coordCount } = useQuery({
    queryKey: ["school-coordinators-count", school.id],
    queryFn: async () => {
  ...
  const createStudentMutation = useMutation({
    mutationFn: async () => {
      const response = await supabase.functions.invoke("create-school-student", {
        body: { school_id: school.id, students: [{ email: studentEmail, password: studentPassword, full_name: studentName }] },
      });
  ...
  const handleImportCSV = async () => {
    ...
    try {
      ...
      const response = await supabase.functions.invoke("create-school-student", { body: { school_id: school.id, students: studentsToCreate } });
      if (response.error) throw new Error(response.error.message);
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Alunos: {students?.length || 0} · Coordenadores: {coordCount || 0}</p>
            </div>
          </div>
        </div>
                      ...
                      {filteredStudents.map((student) => (
                        <TableRow key={student.id}>
                          ...
                          <TableCell className="text-right">
                            <Button variant="ghost" size="icon" onClick={() => setEditingStudent({
                              user_id: student.user_id, full_name: student.profile?.full_name || null,
                              cpf: student.profile?.cpf || null, whatsapp: student.profile?.whatsapp || null,
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>O aluno será desvinculado da escola, mas sua conta não será excluída.</AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
      ...
      <Dialog open={isImportOpen} onOpenChange={setIsImportOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Formato: Nome, Email, Senha (uma linha por aluno). Se a senha for omitida, será usada "123456".</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="rounded-md bg-muted p-3 text-sm text-muted-foreground">
              ...
              <pre className="text-xs">{`Nome,Email,Senha\nJoão Silva,joao@email.com,senha123\nMaria Santos,maria@email.com,`}</pre>
            </div>
            {importResults && (
